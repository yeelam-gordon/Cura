# Windows Arm64 installer fix design — r2
...
The assignment inputs were read verbatim and their SHA-256 values verified:
...
| `DR-PROVENANCE-002` | **Accepted; closed in design.** | One literal workflow PR SHA is passed through the validation callers and used for the installer workflow, package workflow, both locally checked-out composite actions, the setup action's helper checkout, and runner-list script checkout; the package caller explicitly sets `platform_windows_arm64: true`. |
...
- Pass it to `make-runners-list.yml`.
- In `conan-package-create`, clean the workspace, check out `_cura_workflows_action` at that ref, invoke the setup action locally, pass `cleanup_workspace: false`, and pass the same ref as `cura_workflows_branch`.
- Print and assert the resolved SHA when the input is a literal SHA.

#### `.github\workflows\make-runners-list.yml`

Add required/defaulted input `cura_workflows_ref`, use it instead of hardcoded `main` for the `Cura-workflows` checkout, and print the resolved checkout SHA before running the script.
...
2. Push `V` and run the package workflow. The checked-out/package source revision is `V`; the summary must record `C`, `V`, `W`, prove that `C..V` changes only the two caller workflows, and record the resolved setup-action SHA, runner-list checkout SHA, `windows-11-arm`, runner/Python Arm64 identity, and uploaded Conan reference.
...
4. The installer run must record:

| Executed item | Required resolved revision |
|---|---|
| reusable package workflow | `W` |
| reusable installer workflow | `W` |
| setup composite action definition | `W` |
| set-package-overrides action definition | `W` |
| setup action's `Cura-workflows` checkout | `W` |
| runner-list script checkout | `W` |
...
5. Attach the package-run URL, installer-run URL, artifact IDs/digests, unsigned/signed contract hashes, and `C`/`W` map to the PR validation record.
...
This path uses one workflow implementation SHA, `W`, for every changed workflow/action/script, records the exact package checkout `V` and underlying product implementation `C`, and explicitly exercises the corrected Arm package runner.
...
2. Implement the workflow PR and run:
   ```powershell
   python -m pytest runner_scripts\tests\test_windows_arm_artifact_contract.py
...
6. Require CI evidence for:
   ...
   - exact `C`/`W` provenance and artifact contract digests.
...
Justification: The revised design converts every locally controllable critical/high gap into an exact file change, hash-locked contract, command, or test while preserving production-signing, physical-hardware, and stable-publication decisions as explicit external gates.
